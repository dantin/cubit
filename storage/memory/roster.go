...
func (m *Roster) FetchRosterItems(_ context.Context, username string) ([]rostermodel.Item, rostermodel.Version, error) {
	var (
		ris []rostermodel.Item
		rv  rostermodel.Version
	)

	if err := m.inReadLock(func() error {
		var fnErr error
		ris, fnErr = m.fetchRosterItems(username)
		if fnErr != nil {
			return fnErr
		}
...
func rosterItemsKey(username string) string {
	return "rosterItems:" + username
}

func rosterVersionsKey(username string) string {
	return "rosterVersions:" + username
}

func rosterNotificationsKey(username string) string {
	return "rosterNotifications:" + username
}
