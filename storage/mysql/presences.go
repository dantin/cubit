...
func (s *mySQLPresences) FetchPresence(ctx context.Context, jid *jid.JID) (*capsmodel.PresenceCaps, error) {
	...
	switch err {
	case nil:
		return scanPresenceAndCapabilities(rawXML, node, ver, featuresJSON)
	case sql.ErrNoRows:
		return nil, nil
	default:
		return nil, err
...
func (s *mySQLPresences) FetchPresencesMatchingJID(ctx context.Context, jid *jid.JID) ([]capsmodel.PresenceCaps, error) {
	...
	for rows.Next() {
		...
		presenceCaps, err := scanPresenceAndCapabilities(rawXML, node, ver, featuresJSON)
		if err != nil {
			return nil, err
		}
...
func scanPresenceAndCapabilities(rawXML, node, ver, featuresJSON string) (*capsmodel.PresenceCaps, error) {
