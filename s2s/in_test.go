...
func TestInStream_DialbackVerify(t *testing.T) {
	...
	// invalid key
	_, _ = conn.inboundWriteString(`<db:verify id="abcde" from="localhost" to="example.org">abcd</db:verify>`)
	elem = conn.outboundRead()
	require.Equal(t, "db:verify", elem.Name())
	...
	key := kg.generate("localhost", "example.org", "abcde")

	_, _ = conn.inboundWriteString(fmt.Sprintf(`<db:verify id="abcde" from="localhost" to="example.org">%s</db:verify>`, key))
	elem = conn.outboundRead()
	require.Equal(t, "db:verify", elem.Name())
...
func TestInStream_DialbackAuthorize(t *testing.T) {
	...
	_, _ = conn.inboundWriteString(`<db:result to="example.org">abcd</db:result>`)
	_ = outConn.Close()
	elem = conn.outboundRead()
	require.Equal(t, "db:result", elem.Name())
	require.Equal(t, xmpp.ErrorType, elem.Type())
	...
	_, _ = outConn.inboundWriteString(`
	   <db:verify from="example.org" type="valid"/>
	   `)
	elem = conn.outboundRead()
	require.Equal(t, "db:result", elem.Name())
	require.Equal(t, "valid", elem.Type())
...
func TestInStream_SendElement(t *testing.T) {
	...
	fromJID, _ := jid.New("alice", "localhost", "desktop", true)
	toJID, _ := jid.New("bob", "example.org", "desktop", true)

	stm2 := stream.NewMockC2S("123", toJID)
...
version="1.0" xmlns="jabber:server" to="example.org" from="localhost" xmlns:xml="http://www.w3.org/XML/1998/namespace">
`
