...
func NewIQFromElement(e XElement, from *jid.JID, to *jid.JID) (*IQ, error) {
	...
	if (iqType == GetType || iqType == SetType) && e.Elements().Count() != 1 {
		return nil, errors.New(`an IQ stanza of type "get" or "set" must contains one and only child element`)
	}
	if iqType == ResultType && e.Elements().Count() > 1 {
		return nil, errors.New(`an IQ stanza of type "result" must include zero or one child element`)
	}
	iq := &IQ{}
	iq.copyFrom(e)
	iq.SetFromJID(from)
