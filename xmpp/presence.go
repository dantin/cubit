...
func isPresenceType(presenceType string) bool {
	switch presenceType {
	case ErrorType, AvailableType, UnavailableType, SubscribeType, UnsubscribeType, SubscribedType, UnsubscribedType, ProbeType:
		return true
	default:
		return false
	}
...
func (p *Presence) setShow() error {
	...
	case 1:
		if shs[0].Attributes().Count() > 0 {
			return errors.New(" the <show/> element MUST NOT possess any attribute")
		}
		switch shs[0].Text() {
		case "away":
		...
		case "xa":
			p.showState = ExtendedAwayShowState
		default:
