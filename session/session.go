...
const (
	...
	diabackNamespace      = "jabber:server:dialback"
)
...
type Error struct {
	// Element returns the original incoming element that generated the session error.
...
type Config struct {
	...
	// RemoteDomain represents the remote receiving entity domain name.
	RemoteDomain string

	// IsServer defines whether or not this session is established by the server.
	IsServer bool

	// IsInitiating defines whether or not this is an initiating entity session.
	IsInitiating bool
}
...
func (s *Session) Open(ctx context.Context, featuresElem xmpp.XElement) error {
	...
	case transport.Socket:
		...
		if s.isServer {
			ops.SetAttribute("xmlns:db", diabackNamespace)
		}
...
func (s *Session) buildStanza(elem xmpp.XElement) (xmpp.Stanza, *Error) {
	...
	return nil, &Error{Element: elem, UnderlyingErr: streamerror.ErrUnsupportedStanzaType}
}
...
func (s *Session) extractAddresses(elem xmpp.XElement) (*jid.JID, *jid.JID, *Error) {
	...
	if len(to) > 0 {
		...
		toJID = s.jid().ToBareJID() // account's bare JID as defaut 'to'
	}
